...
import { evidenceItemLabel, evidenceTemplate } from "../i18n/strings.ts";
...
export function evidenceItems(input: EvidenceInput, ruleIds: string[]): EvidenceItem[] {
  ...
  // Row labels come from the evidence.<item>.label catalog family; date-bearing values use
  // the last_verified templates with the date interpolated as DATA (spec section 4).
  const L = (key: string) => evidenceItemLabel(key);
  return [
    { key: "device", label: L("device"), state: "ok", quality: qualityFor("device", input, ruleIds), value: input.deviceId ?? "Not recorded", detail: `Source: event record · ${fmtDate(input.timestampUtc)}`, rule: undefined, contributed: false },
    { key: "qc", label: L("qc"), state: input.qcPassed ? "ok" : "fail", quality: qualityFor("qc", input, ruleIds), value: input.qcPassed ? "Passed" : "Failed", detail: `Device ${input.deviceId ?? "not recorded"} · Source: device QC record`, rule: has("QC_FAILED") ? "QC_FAILED → VERIFY" : undefined, contributed: has("QC_FAILED") },
    { key: "cal", label: L("cal"), state: has("CAL_EXPIRED") ? "fail" : has("CAL_NEAR_DUE") ? "warn" : "ok", quality: qualityFor("cal", input, ruleIds), value: input.calibrationDueUtc ? evidenceTemplate("cal", fmtDate(input.calibrationDueUtc)) : "Not recorded", detail: "Source: calibration record", rule: has("CAL_EXPIRED") ? "CAL_EXPIRED → VERIFY" : has("CAL_NEAR_DUE") ? "CAL_NEAR_DUE → REVIEW" : undefined, contributed: has("CAL_EXPIRED") || has("CAL_NEAR_DUE") },
    { key: "op", label: L("op"), state: input.operatorCompetent ? "ok" : "warn", quality: qualityFor("op", input, ruleIds), value: input.operatorId ? `${input.operatorId} · ${input.operatorCompetent ? "competency current" : "competency not current"}` : "Operator not recorded", detail: "Source: operator competency record — prototype data, identity not authenticated.", rule: has("OPERATOR_NOT_COMPETENT") ? "OPERATOR_NOT_COMPETENT → REVIEW" : undefined, contributed: has("OPERATOR_NOT_COMPETENT") },
    { key: "reagent", label: L("reagent"), state: has("REAGENT_EXPIRED") ? "fail" : has("REAGENT_NEAR_EXPIRY") ? "warn" : "ok", quality: qualityFor("reagent", input, ruleIds), value: input.reagentLot ? `${input.reagentLot} · ${evidenceTemplate("reagent", fmtDate(input.reagentExpiryUtc))}` : "Lot not recorded", detail: "Source: consumable record", rule: has("REAGENT_EXPIRED") ? "REAGENT_EXPIRED → VERIFY" : has("REAGENT_NEAR_EXPIRY") ? "REAGENT_NEAR_EXPIRY → REVIEW" : undefined, contributed: has("REAGENT_EXPIRED") || has("REAGENT_NEAR_EXPIRY") },
    { key: "env", label: L("env"), state: has("ENV_TEMP") || has("ENV_HUMIDITY") ? "warn" : "ok", quality: qualityFor("env", input, ruleIds), value: `${input.temperatureC ?? "—"}°C · ${input.humidityPct ?? "—"}%`, detail: input.powerInterruption ? "Power interruption recorded · Source: site sensor" : "No power interruption recorded · Source: site sensor", rule: has("POWER_INTERRUPTION") ? "POWER_INTERRUPTION → REVIEW" : undefined, contributed: has("ENV_TEMP") || has("ENV_HUMIDITY") || has("POWER_INTERRUPTION") },
    { key: "prov", label: L("prov"), state: has("PROVENANCE_INCOMPLETE") ? "warn" : "ok", quality: qualityFor("prov", input, ruleIds), value: input.provenance || "Not fully recorded", detail: `Operator ID as claimed: ${input.operatorId || "not recorded"} · Where: ${input.provenance || "not recorded"} · ${fmtDate(input.timestampUtc)}`, rule: has("PROVENANCE_INCOMPLETE") ? "PROVENANCE_INCOMPLETE → REVIEW" : undefined, contributed: has("PROVENANCE_INCOMPLETE") },
    { key: "conn", label: L("conn"), state: offline ? "warn" : "ok", quality: "valid", value: offline ? "Offline event" : "Online", detail: offline ? "Offline: synchronisation metadata only — the deterministic engine has no connectivity rule in this prototype, so this does not make the result more or less reliable. Full sync engine is FUTURE." : "Prototype offline metadata; full sync engine is FUTURE.", rule: undefined, contributed: false },
  ];
}
